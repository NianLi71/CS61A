...
def choose(paragraphs, select, k):
    ...
    the empty string.
    Test by running: python3 ok --local -q 01
    ...
    if k > current_index:  
        # also works when no match in select
        # then k will be > 0, current_index will be -1
        return ''
...
def about(topic):
    ...
    def topic_checker(paragraph):
        funcs_list = [lower, remove_punctuation, split]
        s = paragraph
        for f in funcs_list:
            s = f(s)
        word_list_for_paragraph = s

        for topic_word in topic:
            for word in word_list_for_paragraph:
                if topic_word == word:
                    return True
        return False
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    correct_cnt = 0
    limit = min(len(sofar), len(prompt))
    for i in range(limit):
        if sofar[i] == prompt[i]:
            correct_cnt += 1
        else:
            break
    progress = float(correct_cnt) / len(prompt)
...
def time_per_word(words, times_per_player):
    ...
    for t in times_per_player:
        ...
        for i in range(len(t) - 1):
            time_per_one_player.append(t[i+1] - t[i])
        time_costs.append(time_per_one_player)

    return match(words, time_costs)
...
def fastest_words(match):
    ...
    fastest_list = [ list() for i in player_indices]
    for w_i in word_indices:
        min_index = 0
        for p_i in player_indices:
            if time(match, p_i, w_i) < time(match, min_index, w_i):
                min_index = p_i
        fastest_list[min_index].append(word_at(match, w_i))
